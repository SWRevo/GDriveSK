...
class AppDataDriveServiceHelper(private val mDriveService: Drive) {
    ...
    fun downloadFile(fileId: String?): Task<InputStream> {
        return Tasks.call(mExecutor, { mDriveService.files()[fileId].executeMediaAsInputStream() })
    }
 ...
 * Developer's Console](https://play.google.com/apps/publish) and be submitted to Google for verification.
    ...
    fun queryFiles(): Task<List<GoogleDriveFileHolder>> {
        return Tasks.call(mExecutor, {
            val googleDriveFileHolderList: MutableList<GoogleDriveFileHolder> = ArrayList()
            val result = mDriveService.files().list().setFields("files(id, name,size,createdTime,modifiedTime,starred,mimeType)").setSpaces("appDataFolder").execute()
            for (i in result.files.indices) {
                val googleDriveFileHolder = GoogleDriveFileHolder()
                googleDriveFileHolder.id = result.files[i].id
                googleDriveFileHolder.name = result.files[i].name
    ...
    fun queryFiles(folderId: String?): Task<List<GoogleDriveFileHolder>> {
        return Tasks.call(mExecutor, {
            ...
            val result = mDriveService.files().list().setQ("'$parent' in parents").setFields("files(id, name,size,createdTime,modifiedTime,starred,mimeType)").setSpaces("appDataFolder").execute()
            for (i in result.files.indices) {
                val googleDriveFileHolder = GoogleDriveFileHolder()
                googleDriveFileHolder.id = result.files[i].id
                googleDriveFileHolder.name = result.files[i].name
    ...
    companion object {
        ...
        fun getGoogleAppDataDriveService(context: Context?, account: GoogleSignInAccount, appName: String?): Drive {
